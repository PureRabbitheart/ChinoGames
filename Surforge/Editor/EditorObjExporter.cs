...
public class EditorObjExporter : ScriptableObject
{
	...
	private static string MeshToString(MeshFilter mf, Dictionary<string, ObjMaterial> materialList) 
	{
		...
		for (int material=0; material < m.subMeshCount; material ++) {
			...
			{	
				//fix maya import bug, prevent to create geometry with zero thickness
				int a = triangles[i]+1 + vertexOffset;
				int b = triangles[i+1]+1 + normalOffset;
				int c = triangles[i+2]+1 + uvOffset;
	...
	static void ExportWholeSelectionToSingle()
	{
		...
		if (exportedObjects > 0)
		{
			...
			string filename = "test" + "_" + exportedObjects;
