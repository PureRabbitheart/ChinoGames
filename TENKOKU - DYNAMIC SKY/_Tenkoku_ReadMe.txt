...
WHAT'S NEW
- Added Cloud Brightness setting to control over brightening of clouds.
- Rewrote some demo scripts from javascript --> C#.
- Removed old Unity 5.0 access.  Minimum version is now Unity 5.3.4.

CHANGES
- humidity level now affects visibility of clouds when set to high levels.
- reflection probe position now updates according to probe render schedule (should prevent some visual stagger discrepancies).
- Edited rainbow rendering so it is always masked by horizon (previously it was masked by humidity/fog depth).

BUG FIXES
- Fixed overcast brightness error of legacy cloud shader when viewed at night.
- Adjusted Shader semantics for PS4 compiling.
- Fixed flickering and improved rendering behavior during Solar Eclipse.
- Removed extra texture sample in legacy cloud shader for Unity 2018.x compatibility.
